...
public class HamiltonianRoute implements IRoutePlanner{
    ...
    private void setupEdge(final List<Edge> edges, final List<Vertex> vertices){
        for (int i = 0; i < vertices.size(); i++){
            for (int j = i + 1; j < vertices.size(); j++){
                edges.add(new Edge(vertices.get(i), vertices.get(j), PathCost(vertices.get(i).position, vertices.get(j).position)));
            }
        }
    }
    ...
    private List<Edge> findEdgesInShortestPath(List<Vertex> vertices, List<Edge> edges){
        ...
        int[] remainingVertices = new int[vertices.size() - 1];
        for (int i : remainingVertices) i = 2;
        ...
        int remainingElements = vertices.size();
        ...
        while(remainingElements != 0){
            ...
            Edge current = queueOfEdges.peek();
            ...
            if (success){
                ...
                if (--remainingVertices[position] == 0)
                    queueOfEdges.removeIf(e ->
                            e.start.equals(current.start) || e.end.equals(current.end));
